...
class VCFWriter:
    ...
    def create_header(self, variants: List[VariantAnalysis], template_vcf: Optional[Path] = None) -> str:
        ...
        if template_vcf:
            ...
            self.write_header_line(buffer, "FILTER=<ID=REPEAT_OVERLAP_MATCH,Description=\"Variant is likely in a RepeatMasker overlap\">")
            self.write_header_line(buffer, "FILTER=<ID=REPEAT_PROXIMAL_MATCH,Description=\"Variant is likely in a RepeatMasker proximal feature\">")
            self.write_header_line(buffer, "FILTER=<ID=L1_NESTING,Description=\"Variant is likely in a L1 nesting region\">")
